...
```{r}
Ymat = yelp.tokens.tfidf.df$stars
yelp.tokens.tfidf.df$stars = NULL
```

```{r}
Xmat = as.matrix(yelp.tokens.tfidf.df)
```
...
```{r}
Xmatnew = as.matrix(yelp.out.tokens.tfidf.df)
```


```{r}
predict(yelp.ridge.cv, newx = Xmatnew, s = "lambda.min") #prediction
```
